        ...
        <div class="single-form-left">
            <div class="contact-single">
                <h3 class="editContent">
                    <span class="sub-tittle editContent">0</span>
                    Оставить отзыв
                </h3>
                <form action="#" method="get" class="mt-4" id="formReview">
                    ...
                    <!-- button потому что мы будем не отправлять форму а вызывать конкретный метод -->
                    <button type="button" class="mt-3 btn btn-success btn-block py-3" @click="sendReview()">
                        Отправить
                    </button>
                </form>
            </div>
...
export default {
    ...
    methods: {
        async sendReview() {
            // сформируем данные, которые будет отправлять на бэкэнд
            let data = {
                name: this.name,
                email: this.email,
                text: this.text,
